...
exports.signUp = (req, res) => {
  ...
  db.doc(`/${newUser.email.split("@")[0]}/mainInfo`)
    .get()
    .then((doc) => {
      if (doc.exists) {
    ...
    .then((tokenId) => {
      ...
      db.collection(`${newUser.email.split("@")[0]}`)
        .doc("mainInfo")
        .set({ userCredentials });
...
exports.login = (req, res) => {
  ...
  firebase
    ...
    .catch((err) => {
      console.error(err);
      return res.status(403).json({ general: `${err.code}`.slice(5) });
    });
};
  ...
  busboy.on("finish", () => {
    admin
      ...
      .then(() => {
        ...
        return db
          .doc(`/${req.user.email.split("@")[0]}/mainInfo`)
          .update({ imageUrl });
      })
      .then(() => {
